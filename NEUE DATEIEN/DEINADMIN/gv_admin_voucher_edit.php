 ...
 * @version $Id: gv_admin_voucher_edit.php 2019-12-04 08:49:16Z webchills $
 */
  ...
  if (zen_not_null($action)) {
    ...
    case 'confirmdelete':
      ...
      if (zen_admin_demo()) {
        ...
        zen_redirect(zen_href_link(FILENAME_GV_ADMIN_VOUCHER_EDIT));
      }
...
<?php
  ...
  while (!$gv_list->EOF) {
    if (((!$_GET['gid']) || (@$_GET['gid'] == $gv_list->fields['customers_id'])) && (!$gInfo)) {
      $gInfo = new objectInfo($gv_list->fields);
    }
    if ( (is_object($gInfo)) && ($gv_list->fields['customers_id'] == $gInfo->customers_id) ) {
